...
export default async function RegistrationPage() {
  ...
  return (
    <>
      ...
      {!event ? (
        <Panel>
          <h2 className="text-xl font-black text-[#F2B705]">Banco de dados indisponivel</h2>
      ...
      <Panel>
        <form action={submitRegistration} className="grid gap-4">
          ...
          <fieldset className="grid gap-3">
            ...
            {games.map((game) => (
              <label key={game.id} className="flex min-h-12 items-center justify-between border border-[#F2B705]/30 bg-black/30 px-3">
                <span>{game.name}</span>
                <span className="flex items-center gap-3 text-[#F2B705]">
          ...
          <button className="focus-ring min-h-12 bg-[#F2B705] px-5 font-black uppercase text-black disabled:opacity-50" disabled={!event} type="submit">Gerar Pix</button>
        </form>
      </Panel>
      </Container>
    </>
