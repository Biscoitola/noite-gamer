  ...
  return (
    <>
      ...
      <Panel className="grid gap-4">
        ...
        <strong className="text-2xl text-[#F2B705]">Status: {registration.status}</strong>
        {payment && registration.status !== "CONFIRMADA" ? (
          <>
            ...
            <textarea className="min-h-28 border border-[#F2B705]/35 bg-black p-3 text-sm" readOnly value={payment.qrCodeText} />
            <p>Valor: R$ {Number(payment.amount).toFixed(2)}</p>
            {payment.provider === "manualpix" ? (
              <div className="border border-[#F2B705]/50 bg-[#F2B705]/10 p-4 text-sm leading-6">
                <strong className="block text-[#F2B705]">Importante para confirmar sua inscricao</strong>
