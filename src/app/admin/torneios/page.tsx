    ...
    <Container className="grid gap-5">
      ...
      <Panel>
        <form action={generateTournamentAction} className="flex flex-wrap gap-3">
          <select name="gameId" className="min-h-12 border border-[#F2B705]/35 bg-black px-3">
            {games.map((game) => <option key={game.id} value={game.id}>{game.name}</option>)}
          </select>
          <label className="flex items-center gap-2"><input name="onlyCheckedIn" type="checkbox" /> Somente check-in</label>
          <button className="min-h-12 bg-[#F2B705] px-4 font-black text-black">Gerar chave</button>
        </form>
      </Panel>

      <Panel className="grid gap-3">
        <h2 className="text-xl font-black text-[#F2B705]">Escolha a chave para editar</h2>
        <div className="flex flex-wrap gap-2">
          {tournaments.map((tournament) => (
            <Link
              className={`border px-4 py-3 text-sm font-black uppercase transition ${
                selectedTournament?.id === tournament.id
                  ? "border-[#F2B705] bg-[#F2B705] text-black"
                  : "border-[#B45CFF]/50 text-[#F5F5F5] hover:border-[#F2B705] hover:text-[#F2B705]"
              }`}
      ...
      {selectedTournament ? (
        <Panel key={selectedTournament.id} className="grid gap-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h2 className="text-xl font-black text-[#F2B705]">{selectedTournament.game.name} - {selectedTournament.status}</h2>
            <Link className="border border-[#B45CFF] px-3 py-2 text-sm font-black uppercase text-[#B45CFF] hover:border-[#F2B705] hover:text-[#F2B705]" href={`/torneios/${selectedTournament.game.slug}`}>
              Ver chave publica
            </Link>
          </div>
            ...
            <div key={round.id}>
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {round.matches.map((match) => (
                  <article key={match.id} className="grid gap-3 border border-[#F2B705]/25 bg-black/35 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <strong>Partida {match.position}</strong>
                      <span className="text-xs font-black uppercase text-[#A3A3A3]">{match.status}</span>
                    </div>
                    {match.winner ? (
                      <p className="border border-[#F2B705]/40 bg-[#F2B705]/10 p-2 text-sm font-black text-[#F2B705]">
...
function WinnerButton({ disabled, entryId, matchId, name }: { disabled: boolean; entryId?: string; matchId: string; name: string }) {
  return (
    <form action={winnerAction}>
      ...
      <button
        className="focus-ring grid min-h-12 w-full grid-cols-[1fr_auto] items-center gap-3 border border-[#B45CFF]/45 bg-[#111111] px-3 text-left font-black text-white transition hover:border-[#F2B705] hover:text-[#F2B705] disabled:cursor-not-allowed disabled:opacity-45"
