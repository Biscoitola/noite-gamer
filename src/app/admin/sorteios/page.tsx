import Link from "next/link";
import { Container, Panel } from "@/components/ui";
import { AdminEventSelector, type AdminSearchParams, getAdminEventFilter, withEventParam } from "@/lib/admin-event-filter";
import { requireAdminRole } from "@/lib/auth";
import { prisma } from "@/lib/db";
import { clearPrizeWinnerAction, drawPrizeAction } from "../patrocinadores/actions";

export const dynamic = "force-dynamic";

export default async function AdminRafflesPage({ searchParams }: { searchParams: Promise<AdminSearchParams> }) {
  await requireAdminRole("ADMIN");
  const params = await searchParams;
  const { events, selectedEventId } = await getAdminEventFilter(params);
  const currentPath = withEventParam("/admin/sorteios", selectedEventId);
  const prizes = await prisma.prize.findMany({
    where: selectedEventId ? { eventId: selectedEventId } : {},
    include: {
      event: true,
      sponsor: true,
      winnerRegistration: { include: { participant: true } }
    },
    orderBy: [{ drawnAt: "desc" }, { createdAt: "desc" }]
  });
  const pending = prizes.filter((prize) => !prize.winnerRegistrationId).length;
  const drawn = prizes.length - pending;
  const tournamentWinnerPrizes = prizes.filter((prize) => prize.raffleAudience === "TOURNAMENT_WINNERS");
  const participantPrizes = prizes.filter((prize) => prize.raffleAudience !== "TOURNAMENT_WINNERS");

  return (
    <Container className="grid gap-5">
      <div>
        <p className="text-sm font-black uppercase text-[#B45CFF]">Brindes e ganhadores</p>
        <h1 className="text-3xl font-black text-glow">Sorteios</h1>
      </div>

      <Message type="success" value={readSearchParam(params.success)} />
      <Message type="error" value={readSearchParam(params.error)} />
      <AdminEventSelector events={events} selectedEventId={selectedEventId} params={params} />

      <section className="grid gap-3 sm:grid-cols-3">
        <Panel>
          <span>Premios</span>
          <strong className="block text-3xl text-[#A855F7]">{prizes.length}</strong>
        </Panel>
        <Panel>
          <span>Aguardando sorteio</span>
          <strong className="block text-3xl text-[#A855F7]">{pending}</strong>
        </Panel>
        <Panel>
          <span>Sorteados</span>
          <strong className="block text-3xl text-[#A855F7]">{drawn}</strong>
        </Panel>
      </section>

      <Panel>
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="text-xl font-black text-[#A855F7]">Premios vinculados a patrocinadores</h2>
            <p className="mt-1 text-sm text-[#A3A3A3]">
              Cadastre os premios nos patrocinadores e escolha o publico do sorteio. Aqui voce sorteia sem misturar campeoes com participantes.
            </p>
          </div>
          <Link className="focus-ring border border-[#B45CFF]/50 px-3 py-2 text-xs font-black uppercase text-[#F5F5F5]" href={withEventParam("/admin/patrocinadores", selectedEventId)}>
            Cadastrar premio
          </Link>
        </div>

        <PrizeGroup
          emptyText="Nenhum premio exclusivo para campeoes cadastrado."
          prizes={tournamentWinnerPrizes}
          returnTo={currentPath}
          title="Sorteio dos campeoes"
        />
        <PrizeGroup
          emptyText="Nenhum premio geral cadastrado."
          prizes={participantPrizes}
          returnTo={currentPath}
          title="Sorteio dos participantes"
        />
      </Panel>
    </Container>
  );
}

type AdminPrize = {
  id: string;
  title: string;
  description: string;
  imageUrl: string;
  quantity: number;
  raffleAudience: string;
  winnerRegistrationId: string | null;
  event: { edition: string };
  sponsor: { name: string };
  winnerRegistration: { participant: { publicName: string }; raffleCode: string | null; protocol: string } | null;
};

function PrizeGroup({ emptyText, prizes, returnTo, title }: { emptyText: string; prizes: AdminPrize[]; returnTo: string; title: string }) {
  return (
    <section className="mt-5 grid gap-3">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#B45CFF]/25 pb-2">
        <h3 className="text-lg font-black text-[#A855F7]">{title}</h3>
        <span className="text-xs font-black uppercase text-[#A3A3A3]">{prizes.length} premio{prizes.length === 1 ? "" : "s"}</span>
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        {prizes.map((prize) => (
          <article className="grid gap-3 border border-[#B45CFF]/30 bg-black/25 p-4" key={prize.id}>
            <div className="flex gap-3">
              <img src={prize.imageUrl} alt={prize.title} className="h-24 w-24 border border-[#A855F7]/30 object-cover" />
              <div className="min-w-0">
                <p className="text-xs font-black uppercase text-[#B45CFF]">{prize.event.edition} | {prize.sponsor.name}</p>
                <h4 className="text-xl font-black text-[#A855F7]">{prize.title}</h4>
                <p className="mt-1 text-sm text-[#A3A3A3]">{prize.description}</p>
                <p className="mt-2 text-xs font-black uppercase text-[#D4D4D4]">Quantidade: {prize.quantity}</p>
                <p className="mt-1 text-xs font-black uppercase text-[#A855F7]">{raffleAudienceLabel(prize.raffleAudience)}</p>
              </div>
            </div>

            {prize.winnerRegistration ? (
              <div className="border border-emerald-400/35 bg-emerald-400/10 p-3 text-sm">
                <p className="text-xs font-black uppercase text-emerald-200">Ganhador</p>
                <strong className="text-lg text-emerald-100">{prize.winnerRegistration.participant.publicName}</strong>
                <p className="text-[#D4D4D4]">Ticket: {prize.winnerRegistration.raffleCode ?? prize.winnerRegistration.protocol}</p>
              </div>
            ) : (
              <div className="border border-[#A855F7]/30 bg-[#A855F7]/10 p-3 text-sm font-black uppercase text-[#A855F7]">
                Aguardando sorteio
              </div>
            )}

            <div className="grid gap-2 sm:grid-cols-2">
              <form action={drawPrizeAction}>
                <input name="prizeId" type="hidden" value={prize.id} />
                <input name="returnTo" type="hidden" value={returnTo} />
                <button className="focus-ring neon-action min-h-11 w-full px-3 text-xs font-black uppercase">
                  {prize.winnerRegistration ? "Sortear novamente" : "Sortear"}
                </button>
              </form>
              <form action={clearPrizeWinnerAction}>
                <input name="prizeId" type="hidden" value={prize.id} />
                <input name="returnTo" type="hidden" value={returnTo} />
                <button className="focus-ring min-h-11 w-full border border-[#B45CFF]/50 px-3 text-xs font-black uppercase text-[#F5F5F5]">
                  Limpar ganhador
                </button>
              </form>
            </div>
          </article>
        ))}
        {prizes.length === 0 ? <p className="text-sm text-[#A3A3A3]">{emptyText}</p> : null}
      </div>
    </section>
  );
}

function Message({ type, value }: { type: "success" | "error"; value?: string }) {
  if (!value) return null;
  return (
    <div className={type === "success" ? "border border-emerald-400/40 bg-emerald-400/10 p-3 text-sm font-black text-emerald-100" : "border border-red-400/45 bg-red-500/10 p-3 text-sm font-black text-red-100"}>
      {value}
    </div>
  );
}

function readSearchParam(value: string | string[] | undefined) {
  return Array.isArray(value) ? value[0] : value;
}

function raffleAudienceLabel(value: string) {
  return value === "TOURNAMENT_WINNERS" ? "Somente campeoes dos torneios" : "Todos os inscritos confirmados";
}
