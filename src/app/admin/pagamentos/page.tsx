...
export default async function PaymentsPage({ searchParams }: { searchParams?: Promise<AdminSearchParams> }) {
  ...
  const currentPath = withEventParam("/admin/pagamentos", selectedEventId);
  const payments = await prisma.payment.findMany({
    where: selectedEventId ? { registration: { eventId: selectedEventId } } : {},
  ...
  return (
    <Container className="grid gap-5">
      ...
      <Panel>
        <h2 className="text-xl font-black text-[#A855F7]">Conciliacao por PDF</h2>
        <p className="mt-2 text-sm leading-6 text-[#D4D4D4]">
          ...
          quando valor e protocolo, referencia ou nick baterem com a inscricao.
        </p>
        <div className="mt-4">
          <OfxImportForm />
        </div>
      ...
      <Panel className="grid gap-3">
        {payments.map((payment) => (
          <div key={payment.id} className="grid gap-3 border-b border-[#A855F7]/20 py-3 md:grid-cols-[1fr_auto] md:items-center">
            <div>
